...
| GET | `/exportZones` | Download records as CSV or Excel (`format=csv\|xlsx`); `zone=` for one zone, omit for all. Records only — **no DNSSEC keys or Smart IP rules** (use `/exportDB` for those) |
| POST | `/importZones` | Bulk-create zones + records from an uploaded CSV/XLSX (multipart `file`); additive; returns a JSON ImportReport. DNSSEC keys are not imported — restore via `/exportDB` |
...
- **Import / Export dialog** — a single "Import / Export" button on the Hosted Zones page opens a two-pane modal: export (records as CSV/Excel via `/exportZones`, or a full restorable database snapshot via `/exportDB`) on the left, bulk import (`/importZones`) on the right. **Records export/import is content-only — DNSSEC keys and Smart IP rules are not carried; the full database snapshot (`/exportDB`) is the only thing that preserves keys.** Downloads honor the server's `Content-Disposition` filename and fall back to a `scleraDNS-…` name. CSV uploads have a leading UTF-8 BOM stripped client-side before sending, because Go's `encoding/csv` rejects a quoted first column preceded by a BOM ("bare quote in non-quoted field"). A successful import toasts, refreshes the zone list, and closes the dialog.
